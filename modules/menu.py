...
class main:
  def install_tools(self):
    while True:
      ...
      print(f"")
      logo.back()
      cmd=input(f"{blue}Dig-Up{nc}@{blue}space {yellow}$ {nc}")
        ...
        try:
          if int(cmd) in range(1,int(total)+1):
            while True:
              ...
              print(f"")
              logo.back()
              tcmd=input(f"{blue}Dig-Up{nc}@{blue}space {yellow}$ {nc}")
  ...
  def menu(self):
    while True:
      ...
      if cmd == "1":
        ...
        logo.exit()
        break
      elif cmd=="rm -t" or cmd=="rm -T" or cmd=="uninstall dig-Up" or cmd=="unistall Dig-Up":
        system=sys()
        if system.sudo:
          os.system(system.sudo+" rm -rf "+system.bin+"/Dig-Up")
          os.system(system.sudo+" rm -rf "+system.bin+"/digup")
          os.system(system.sudo+" rm -rf "+system.conf_dir+"/Dig-Up")
        else:
          os.system("rm -rf "+system.bin+"/Dig-Up")
          os.system("rm -rf "+system.bin+"/digup")
          os.system("rm -rf "+system.conf_dir+"/Dig-Up")
        os.system("clear")
        logo.exit()
        break
      else:
...
class tools:
  ...
  def __init__(self):
    system=sys()
    with open(system.conf_dir+"/Dig-Up/core/data.json") as data_file:
      self.data=json.load(data_file)
    with open(system.conf_dir+"/Dig-Up/core/cat.json") as cat_file:
